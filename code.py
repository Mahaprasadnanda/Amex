#!/usr/bin/env python3
"""
ULTIMATE GPU-OPTIMIZED AMERICAN EXPRESS CAMPUS CHALLENGE 2025 SOLUTION
======================================================================
Aims for AUC >0.99999 with full ensemble including TabNet on ROCm GPU.
Uses LightGBM, XGBoost, CatBoost, and TabNet with Optuna tuning and stacking.
Expected runtime: 1-2 hours with GPU acceleration.
"""
...
print("🚀 ULTIMATE GPU-OPTIMIZED SOLUTION FOR AUC >0.99999")
print("=" * 80)
print("✅ Full ensemble with TabNet on ROCm GPU for top rank!")
print("✅ Advanced features, Optuna tuning, stacking for high score!")
print("=" * 80)
...
class OptimizedAmexSolution:
    ...
    def aggregate_additional_data(self, df, filename):
        ...
        try:
            ...
            agg_df = self.optimize_memory(agg_df)
            return agg_df
        except Exception as e:
    ...
    def load_all_data(self):
        ...
        if self.train_df is not None:
            self.train_df = self.optimize_memory(self.train_df)
        
        self.test_df = self.safe_read_parquet('test_data.parquet')
        if self.test_df is not None:
            self.test_df = self.optimize_memory(self.test_df)
        ...
        if self.additional_dfs['offer_metadata'] is not None:
            self.additional_dfs['offer_metadata'] = self.optimize_memory(self.additional_dfs['offer_metadata'])
        
        add_trans = self.safe_read_parquet('add_trans.parquet', use_dask=True)
        add_event = self.safe_read_parquet('add_event.parquet', use_dask=True)
        
        trans_agg = self.aggregate_additional_data(add_trans, 'add_trans.parquet')
        event_agg = self.aggregate_additional_data(add_event, 'add_event.parquet')
    ...
    def tune_model(self, trial, X, y, model_type):
        ...
        if model_type == 'lgb':
            ...
            model = cb.CatBoostClassifier(**params, random_state=42, verbose=0, task_type='GPU' if use_gpu else 'CPU')
            ...
            model = TabNetClassifier(**params, seed=42, device_name='cuda' if use_gpu else 'cpu')
        
        for train_idx, val_idx in cv.split(X, y):
            X_train, X_val = X.iloc[train_idx], X.iloc[val_idx]
            y_train, y_val = y.iloc[train_idx], y.iloc[val_idx]
            if model_type == 'tabnet':
                model.fit(X_train.values, y_train.values, eval_set=[(X_val.values, y_val.values)], patience=5, max_epochs=20, eval_metric=['auc'])
            else:
    ...
    def train_model(self):
        ...
        # Downsample for tuning (20% to balance speed and accuracy)
        sample_frac = 0.2
        X_sample = X.sample(frac=sample_frac, random_state=42)
        y_sample = y.loc[X_sample.index]
        
        # Class weights - FIXED: Correct usage
        weights = compute_class_weight(class_weight='balanced', classes=np.unique(y), y=y)
        class_weights = dict(zip(np.unique(y), weights))
        
        # Optuna tuning with reduced trials and n_jobs=4
        optuna.logging.set_verbosity(optuna.logging.WARNING)
        
        study_lgb = optuna.create_study(direction='maximize')
        study_lgb.optimize(lambda trial: self.tune_model(trial, X_sample, y_sample, 'lgb'), n_trials=10, n_jobs=4)
        lgb_params = study_lgb.best_params
        
        study_xgb = optuna.create_study(direction='maximize')
        study_xgb.optimize(lambda trial: self.tune_model(trial, X_sample, y_sample, 'xgb'), n_trials=10, n_jobs=4)
        xgb_params = study_xgb.best_params
        
        study_cat = optuna.create_study(direction='maximize')
        study_cat.optimize(lambda trial: self.tune_model(trial, X_sample, y_sample, 'cat'), n_trials=10, n_jobs=4)
        ...
        # Stacking ensemble with reduced CV and n_jobs=4
        stacking_model = StackingClassifier(estimators=base_models, final_estimator=lgb.LGBMClassifier(random_state=42), cv=3, n_jobs=4)
